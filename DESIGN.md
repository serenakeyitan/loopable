...
4. **One rules file.** `loopable.md` is the single source for both hosts (Codex receives a condensed one-line digest of it).
...
```
loopable.md (judgment rules + reference library, human-edited,
...
- `SessionStart` (Claude): cleanup TTL state, inject `<loopable-rules>` wrapping loopable.md, mark the session injected.
...
3. False positives → judgment etiquette in loopable.md (once per session, stay quiet when unsure) + mute. Over-suggesting is a rules-wording bug; fix the md, verify by live E2E.
